...
const extraTab2String: LocaleStringKey = config.extraTab2?.tabTitleKey ?? 'title_receive'
...
const title: { readonly [key: string]: string } = {
  ...
  extraTab2: s.strings.title_receive
}
...
export const MenuTabs = (props: BottomTabBarProps) => {
  ...
  const handleOnPress = useHandler((route: string) => {
    ...
    switch (route) {
      ...
      case 'extraTab2':
        return navigation.navigate('extraTab2', undefined)
    }
  })
      ...
      <LinearGradient colors={colors} start={start} end={end} style={styles.container}>
        {routes.map((route, index: number) => {
          ...
          const icon: { readonly [key: string]: JSX.Element } = {
            ...
            extraTab2: <VectorIcon font="Feather" name="arrow-down-left" size={theme.rem(1.25)} color={color} />
          }
          return (
